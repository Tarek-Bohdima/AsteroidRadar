...
- **Two API endpoints, two response shapes.** `neo/rest/v1/feed` returns a JSON string parsed manually (`parseAsteroidsJsonResult` in `network/NetworkUtils.kt`) because of its nested-by-date structure; `planetary/apod` is decoded by Moshi into `ImageOfTheDay`. The Retrofit builder registers `ScalarsConverterFactory` first and `MoshiConverterFactory` second; Retrofit walks factories in order, so `String`-returning methods (`getAsteroids`) get the raw body and class-returning methods (`getImageOfDay`) get Moshi. `ImageOfTheDay` carries `@JsonClass(generateAdapter = true)` and the `moshi-kotlin-codegen` KSP processor generates `ImageOfTheDayJsonAdapter` at compile time — no runtime reflection, so R8 can fully strip kotlin-reflect.
...
- **KSP, not kapt, for Room.** Room compiler runs through KSP (`com.google.devtools.ksp`); generated sources are added to `main` via `sourceSets { main { java { srcDir 'build/generated/ksp/src/main/kotlin' } } }`. `kotlin-kapt` is still applied because AGP 8.3.0's Data Binding compiler discovers `@BindingAdapter` methods via kapt; it can be dropped once we're on AGP 8.6+, where Data Binding runs through KSP.
